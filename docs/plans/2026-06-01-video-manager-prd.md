...
- 关键词：匹配文件名/路径。
- 主题背景图：按 `theme_background_id` 筛选；从主题背景图列表页跳转时自动带入该条件。
...
- AC-5.7 按喜爱度排序时顺序正确。
- AC-5.8 从主题背景图列表页点击某背景图后，视频列表仅展示关联该主题背景图的视频。
...
**描述：** 系统提供「主题背景图」独立资源管理。用户可在视频详情页选择某一帧生成主题背景图，也可将已有背景图关联到视频。主题背景图用于辅助视频归档与视觉识别。

**规则：**

- 主题背景图存储在 `data/assets/theme-backgrounds/` 下。
- 每个视频最多关联 1 张主题背景图。
- 每张主题背景图可以关联多个视频。
- 视频可以没有主题背景图。
- 用户在视频详情页播放/暂停或拖动到目标画面后，可将当前帧保存为主题背景图。
- 主题背景图名称全局唯一；支持手工命名或自动命名（格式 `room_0001`、`room_0002` …）。
- 手工命名允许中文、字母、数字、下划线与连字符。
- `name` 为空时系统自动生成下一个可用 `room_XXXX` 名称。
- 视频再次从帧创建新背景图时，替换该视频当前关联；旧背景图若仍被其他视频使用则保留。
- 主题背景图列表页支持按名称搜索；点击某背景图跳转到视频库并筛选关联该背景图的所有视频。
- 删除主题背景图时解除所有视频关联并删除磁盘文件。

**优先级：** P1

**验收标准：**

- AC-14.1 视频详情页可将当前播放帧保存为主题背景图，成功后展示关联的背景图。
- AC-14.2 手工输入重复名称时拒绝保存并提示。
- AC-14.3 不输入名称时自动生成 `room_XXXX` 唯一名称。
- AC-14.4 同一背景图可被多个视频关联；每个视频最多关联一张背景图。
- AC-14.5 主题背景图列表页可搜索名称，点击后跳转视频库并筛选关联视频。
- AC-14.6 图片文件存储于 `data/assets/theme-backgrounds/`，不在扫描目录内。
